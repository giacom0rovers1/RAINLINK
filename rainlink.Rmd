...
accu1hr <- function(CmlRainfall, TIMESTEP){
  ...
  for(selid in ids){                       # Ciclo sugli ID
    ...
    for(seldt in dts){                         # Ciclo sui TIMESTEP
      ...
      elapsed_t <- round((proc.time()-lapTime)[3],digits=1)
      remaining_t <- elapsed_t
      cat(sprintf("\r%d of %d, remaining: %.1f", numdts, totdts, elapsed_t/numdts*(totdts-numdts)))
    }
    
    elapsed_t <- round((proc.time()-startTime)[3],digits=1)
    remaining_t <- elapsed_t
    cat(sprintf("\r%d of %d, remaining: %.1f\n", numids, totids, elapsed_t/numids*(totids-numids)))
  }
